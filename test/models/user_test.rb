...
class UserTest < ActiveSupport::TestCase
  ...
  test "email validation should accept valid adress" do
    valid_adress = %w[user@example.com USER@foo.com A_US-ER@foo.bar.org first.last@foo.jp alice+bob@baz.cn]
    valid_adress.each do |valid_adress|
  ...
  test "email validation should reject invalid adress" do
    invalid_adress = %w[user@example,com user_at_foo.org user.name@example. foo@bar_baz.com foo@bar+baz.com]
    invalid_adress.each do |invalid_adress|
